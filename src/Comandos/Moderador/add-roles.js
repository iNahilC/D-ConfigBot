...
module.exports = new Comando({
  nombre: "add-roles",
  alias: ["addroles", "addrol", "add-rol"],
  categoria: "Moderador",
  descripcion: "Este comando te permite agregar múltiples roles a una persona especifica.",
  ejemplo: "$add-rol [@usuario] [roles]",
  ejecutar: async (client, message, args) => {
    ...
    if (!args[0]) {
      let e = new MessageEmbed();
      e.setColor(client.colorDefault);
      e.setDescription(`${client.emojiError} Necesitas mencionar al **__usuario__** que quieres agregarle los roles.`);
      return message.reply({ embeds: [e], allowedMentions: { repliedUser: false } });
    }
    ...
    if (userRolePossition <= rolePosition) {
      let f = new MessageEmbed()
      f.setColor(client.colorDefault).setDescription('' + client.emojiError + ' No puedes agregarle roles a ese miembro porque tiene roles que son más altos o iguales que tu.')
      return message.reply({ embeds: [f], allowedMentions: { repliedUser: false } });
    } else if (botRolePossition <= rolePosition) {
      let f = new MessageEmbed()
      f.setColor(client.colorDefault).setDescription('' + client.emojiError + ' No puedo agregarle roles a ese miembro porque tiene roles que son más altos o iguales que los mios mí.')
      return message.reply({ embeds: [f], allowedMentions: { repliedUser: false } });
    }
    ...
    if (rol_mencion.length < 1) {
      let e = new MessageEmbed();
      e.setColor(client.colorDefault);
      e.setDescription(`${client.emojiError} Necesitas mencionar los **roles** que quieres **__agregarle__** a el usuario.`);
      return message.reply({ embeds: [e], allowedMentions: { repliedUser: false } });
    }
    ...
    let roles_gramatica = "";
    if (rol_mencion.length == 1) roles_gramatica = `${client.warning} Deseas agregarle el siguiente rol <@&${rol_mencion[0].replace("<@&", "").replace(">", "")}> al usuario ${user}?`;
    if (rol_mencion.length > 1) roles_gramatica = `${client.warning} Deseas agregarle los siguientes **__${rol_mencion.length}__** roles al usuario ${user}?`;
    for (var i = 0; i < rol_mencion.length; i++) {
      ...
      if (rol_mencion.length == 1 && user.roles.cache.has(rol_replaced)) {
        let e = new MessageEmbed();
        e.setColor(client.colorDefault);
        e.setDescription(`${client.emojiError} El usuario ${user} tiene el único rol mencionado para agregar. <@&${rol_replaced}>`);
        return message.reply({ embeds: [e], allowedMentions: { repliedUser: false } });
      }
    }
        ...
        switch (button.customId) {
          case si_id:
            ...
            for (var i = 0; i < rol_mencion.length; i++) {
              ...
              await user.roles.add(rol_mencion[i].replace("<@&", "").replace(">", "")).catch(async (err) => {
            ...
            console.log(roles_array_added)
            let e = new MessageEmbed();
            e.setColor(client.colorDefault);
            if (roles_array_removed.length > 0 && roles_array_added.length > 0) {
              lchannel.send({ content:`
\`\`\`md
# D-ConfigBot [Add-Role | ${roles_array_added.length + roles_array_removed.length} Total]
...
\`\`\``});
              e.setDescription(`${client.emojiSuccess} Los siguientes **__roles__** fueron agregados al usuario ${user}
\`\`\`md
# Agregados [${roles_array_added.length}]

${roles_array_added.map((x) => `* ${message.guild.roles.cache.get(x).name}`).join("\n")}

# No Agregados [${roles_array_removed.length}]
...
\`\`\``);
              ...
              lchannel.send(`
\`\`\`md
# D-ConfigBot [Add-Role | ${roles_array_added.length} Total]
...
\`\`\``);
              ...
              e.setDescription(`${client.emojiSuccess} Los siguientes **__roles__** fueron no fueron agregados al usuario ${user}
\`\`\`md
# No Agregados [${roles_array_removed.length}]
